...
function AppleLogo({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={`h-9 w-9 md:h-[60px] md:w-[60px] transition-colors ${active ? "text-neutral-900" : "text-neutral-400"}`} aria-hidden="true">
...
function SamsungLogo({ active }: { active: boolean }) {
  return (
    <span className={`text-[12px] md:text-[17px] font-black tracking-[0.15em] transition-colors ${active ? "text-[#1428A0]" : "text-neutral-400"}`}>
      SAMSUNG
    </span>
  );
}
...
function XiaomiLogo({ active }: { active: boolean }) {
  return (
    <span className={`flex h-9 w-9 md:h-[46px] md:w-[46px] items-center justify-center rounded-[8px] md:rounded-[10px] text-[14px] md:text-[18px] font-black transition-colors ${active ? "bg-[#FF6900] text-white" : "bg-neutral-200 text-neutral-500"}`}>
      MI
    </span>
  );
}
...
function ZubehoerLogo({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" className={`h-9 w-9 md:h-[60px] md:w-[60px] transition-colors ${active ? "text-neutral-900" : "text-neutral-400"}`} aria-hidden="true">
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative flex h-[68px] min-w-[68px] shrink-0 items-center justify-center rounded-2xl px-3 transition-all duration-200 md:h-[88px] md:min-w-[88px] md:px-5 ${
                  isActive
